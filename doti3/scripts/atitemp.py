...
class Py3status:
    ...
    def empty(self, i3status_output_json, i3status_config):
        ...
        """
        response = {'full_text': "GPU: " + re.sub( r'[\'b]+', '', str(check_output("aticonfig --odgt | awk '{match($0,\"Temperature - ([0-9.]+) C\", a)}END{print a[1]};'| xargs echo -n", shell=True))) + " °C", 'name': 'atitemp', 'instance': 'first'}
        return (0, response)
